...
export const DepositPage: React.FC = () => {
  ...
  console.log(
    "DepositPage статусы: authLoading=",
    authLoading,
    "isAuthenticated=",
    isAuthenticated
  );
  ...
  const fetchCurrencies = useCallback(async () => {
    ...
    try {
      ...
      if (!Array.isArray(cryptoData)) {
        console.error(
          "DepositPage: данные о криптовалютах не являются массивом:",
          cryptoData
        );
      ...
      aggregatedCurrencies.forEach((currency) => {
        console.log(
          `DepositPage: Валюта ${currency.symbol} - icon: "${currency.icon}"`
        );
        if (!currency.icon || currency.icon.trim() === "") {
          console.warn(
            `DepositPage: У валюты ${currency.symbol} отсутствует или пуста иконка`
          );
        }
      });
      ...
      console.log("DepositPage: параметры URL:", {
        walletIdParam,
        cryptoParam,
      });

      if (cryptoParam) {
        const matchedCrypto = aggregatedCurrencies.find(
          (c: Currency) => c.symbol.toLowerCase() === cryptoParam.toLowerCase()
        );

        if (matchedCrypto) {
          console.log(
            "DepositPage: найдена криптовалюта из URL:",
            matchedCrypto
          );
          ...
          const netsArr = Array.from(
            new Set([...(matchedCrypto.networks || []), ...walletNetworks])
          );
          ...
          console.warn(
            "DepositPage: криптовалюта из URL не найдена:",
            cryptoParam
          );
        }
      }
      ...
      if (
        ...
        setError(
          "Нет подключения к серверу. Проверьте ваше интернет-соединение и повторите попытку."
        );
      }
    }
  }, [searchParams]);
  ...
  const fetchUserWallets = useCallback(async () => {
    try {
      ...
      console.error(
        "DepositPage: не удалось загрузить кошельки пользователя",
        err
      );
    }
  }, []);
  ...
  const handleCurrencySelection = useCallback(
    (symbol: string) => {
      ...
      const netsArr = Array.from(
        new Set([...walletNetworks, ...referenceNetworks])
      );
      console.log("DepositPage: доступные сети для валюты:", netsArr);
      console.log(
        "DepositPage: сети из кошельков пользователя:",
        walletNetworks
      );
      ...
      if (netsArr.length === 1) {
        console.log(
          "DepositPage: автоматический выбор единственной сети:",
          netsArr[0]
        );
        setSelectedNetwork(netsArr[0]);
      } else if (netsArr.length > 1) {
        console.log(
          "DepositPage: доступно несколько сетей, пользователь должен выбрать"
        );
        ...
        if (walletNetworks.length === 1) {
          console.log(
            "DepositPage: автоматический выбор сети из кошелька пользователя:",
            walletNetworks[0]
          );
  ...
  const handleRequestDeposit = useCallback(
    async (e: React.MouseEvent) => {
      e.preventDefault();
      console.log(
        "DepositPage: запрос адреса для пополнения:",
        selectedCurrency,
        selectedNetwork
      );
      ...
      let networkToUse =
        selectedNetwork || (allNetworks.length === 1 ? allNetworks[0] : null);
      ...
      try {
        ...
        if (selectedNetwork) {
          ...
          if (!currency) {
            const userWalletForNetwork = userWallets.find(
              (w) =>
            ...
            if (userWalletForNetwork) {
              ...
              currency = availableCurrencies.find(
                (c) => c.id === userWalletForNetwork.currency.id
              );
              console.log(
                "DepositPage: найдена валюта по кошельку пользователя:",
                {
          ...
          if (!currency) {
            console.log(
              "DepositPage: пытаемся найти валюту по точному соответствию сети:",
              selectedNetwork
            );
            // Ищем валюту, которая точно соответствует выбранной сети
            const exactMatchCurrency = availableCurrencies.find(
              (c) =>
                c.symbol === selectedCurrency &&
                c.networks &&
                c.networks.some(
            ...
            if (exactMatchCurrency) {
              ...
              console.log(
                "DepositPage: найдена валюта по точному соответствию сети:",
                {
        ...
        if (!currency) {
          ...
          console.log(
            "DepositPage: fallback - найдена валюта только по символу:",
            {
        ...
        console.log(
          "DepositPage: доступные валюты для отладки:",
        ...
        console.log(
          "DepositPage: кошельки пользователя для отладки:",
          userWallets.map((w) => ({
            id: w.id,
        ...
        const data = response;
        console.log(
          "DepositPage: успешно получена информация для депозита:",
          data
        );
        ...
        toast.dismiss();
        console.error(
          "DepositPage: ошибка при получении информации для депозита:",
          err
        );
        ...
        if (
          ...
          errorMessage =
            "Неверные параметры запроса. Пожалуйста, выберите другую валюту или сеть.";
        } else if (
    ...
    const init = async () => {
      try {
        if (authLoading) {
          ...
          if (!currentAuth) {
            console.log(
              "DepositPage: Пользователь не авторизован, редирект на логин"
            );
            router.push("/login?redirect=/funds/deposit");
            return;
          }
        ...
        if (!authLoading && isAuthenticated) {
          console.log(
            "DepositPage: Пользователь авторизован, загружаем данные"
          );
          setIsLoading(true);
  ...
  useEffect(() => {
    if (!isLoading && availableCurrencies.length > 0) {
      ...
      console.log("DepositPage: параметры в URL:", {
        walletIdParam,
        cryptoParam,
      });
      ...
      if (cryptoParam) {
        ...
        let matchedCrypto = availableCurrencies.find(
          (c) => c.symbol.toLowerCase() === cryptoParam.toLowerCase()
        );

        // Если точного совпадения нет, ищем по частичному совпадению
        if (!matchedCrypto) {
          matchedCrypto = availableCurrencies.find(
            (c) =>
        ...
        if (matchedCrypto) {
          ...
          if (matchedCrypto.networks && matchedCrypto.networks.length > 0) {
            console.log(
              "DepositPage: доступные сети для валюты:",
              matchedCrypto.networks
            );
            ...
            if (matchedCrypto.networks.length === 1) {
              console.log(
                "DepositPage: автоматический выбор сети (одна опция):",
                matchedCrypto.networks[0]
              );
              ...
              const timer = setTimeout(() => {
                ...
                if (button && button instanceof HTMLButtonElement) {
                  console.log(
                    "DepositPage: автоматический клик по кнопке запроса адреса"
                  );
                  button.click();
                } else {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      if (depositInfo.memo) {
        ...
        // Убедимся, что адрес закодирован для URL
        const encodedAddress = encodeURIComponent(depositInfo.address);
        wsUrl = `${wsBase}/ws/deposit_status/address/${encodedAddress}/`;
      } else {
        return;
      }
      ...
      try {
        ...
        ws.onmessage = (event) => {
          try {
            ...
            console.log("Expected address:", depositInfo.address);
            console.log("Expected memo:", depositInfo.memo);
            ...
            if (
              depositInfo.memo &&
              data.memo === depositInfo.memo &&
              data.status
            ) {
              if (data.status === "used") {
                console.log("Deposit confirmed via memo:", data.memo);
                setStatus("completed");
                localStorage.removeItem(DEPOSIT_INFO_KEY);
            ...
            // Для адреса
              ...
              depositInfo.address &&
              data.address === depositInfo.address &&
              data.status
            ) {
              if (data.status === "used") {
                console.log("Deposit confirmed via address:", data.address);
                setStatus("completed");
                localStorage.removeItem(DEPOSIT_INFO_KEY);
                toast.success("Пополнение успешно зачислено!", {
                  duration: 5000,
              ...
              // Log when conditions don't match
              console.log("WebSocket message didn't match expected criteria");
              console.log(
                "depositInfo.address && data.address === depositInfo.address:",
                depositInfo.address && data.address === depositInfo.address
              );
              console.log(
                "depositInfo.memo && data.memo === depositInfo.memo:",
                depositInfo.memo && data.memo === depositInfo.memo
              );
              console.log("data.status:", data.status);
            }
        ...
        {showBypass && (
          <div className="mt-8 text-center">
            <p className="text-amber-400 mb-3">
              Авторизация занимает больше времени, чем обычно
            </p>
            <button
  ...
  if (!isAuthenticated && !depositInfo) {
    console.log(
      "DepositPage: не авторизован и нет активного пополнения, редирект"
    );
    return null;
  }
        ...
        return (
          <div className="flex flex-col items-center justify-center p-8">
            ...
            <p className="mt-4 text-gray-300">
              Загрузка данных для пополнения...
            </p>
          </div>
        );
                      ...
                      {currency.icon && currency.icon.trim() !== "" ? (
                        (() => {
                          ...
                          try {
                            ...
                            return (
                              <>
                                <Image
                                  ...
                                  onError={(e) => {
                                    console.error(
                                      "DepositPage: Ошибка загрузки иконки валюты:",
                                      iconUrl
                                    );
                            ...
                            console.error(
                              "DepositPage: Некорректный URL иконки валюты:",
                              iconUrl,
                              error
                            );
                            return (
              ...
              <div className="mb-6">
                ...
                {networkOptions.length > 1 && (
                  <p className="text-xs text-yellow-500 mt-2">
                    ...
                    Важно: выберите ту же сеть, которую будете использовать для
                    перевода
                  </p>
                )}
              </div>
          ...
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-md">
              ...
              {selectedCurrency &&
                (() => {
                  ...
                  if (!iconPath || iconPath.trim() === "") {
                    console.warn(
                      "DepositPage: Пустой путь к иконке для валюты:",
                      selectedCurrency
                    );
                    return (
                  ...
                  try {
                    ...
                    console.error(
                      "DepositPage: Некорректный URL для изображения:",
                      fullImageUrl,
                      error
                    );
                    return (
                  ...
                  return (
                    <>
                      <Image
                        ...
                        onError={(e) => {
                          console.error(
                            "DepositPage: Ошибка загрузки изображения:",
                            fullImageUrl
                          );
            ...
            <div className="mb-4">
              ...
              <div className="mt-1 relative">
                ...
                <button
                  onClick={() =>
                    copyToClipboard(depositInfo.address, "address")
                  }
            ...
            {depositInfo.requires_memo && (
              <>
                <div className="mb-6">
                  <label
                    htmlFor="deposit-memo"
                    className="block text-sm font-medium text-gray-400"
                  >
                    MEMO (обязательно для зачисления):
                  </label>
                  <div className="mt-1 relative">
                    <input
                      id="deposit-memo"
                    ...
                    <button
                      onClick={() =>
                        copyToClipboard(depositInfo.memo || "", "memo")
                      }
                      className="absolute right-2 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-white"
                ...
                <div className="bg-yellow-900 border-l-4 border-yellow-500 text-yellow-200 p-4 rounded-md mb-6">
                  ...
                  <ul className="list-disc pl-5 mt-2 space-y-1 text-sm">
                    <li>
                      ...
                      в сети {selectedNetwork}.
                    </li>
                    <li>
                      Обязательно укажите MEMO в комментарии к транзакции.
                    </li>
                    <li>
                      Средства, отправленные без MEMO, могут быть утеряны.
                    </li>
                    <li>
                ...
                <div className="bg-blue-900 border-l-4 border-blue-500 text-blue-200 p-4 rounded-md mb-6">
                  ...
                  <ol className="list-decimal pl-5 mt-2 space-y-1 text-sm">
                    <li>
                      Скопируйте адрес и MEMO, нажав на соответствующие кнопки.
                    </li>
                    <li>Откройте ваш внешний кошелек или биржу.</li>
                    <li>Создайте новый перевод на указанный адрес.</li>
                    <li>
                      Обязательно укажите MEMO в поле комментария/примечания.
                    </li>
                    <li>
                      После отправки средств дождитесь подтверждения сети.
                    </li>
                  </ol>
                </div>
              </>
